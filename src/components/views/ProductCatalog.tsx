      ...
      {products.map((product) => {
        ...
        return (
          <Link
            ...
            className="cb-card block transition hover:border-forest-mid"
          >
            ...
            <p className={`mt-2 text-sm font-bold ${pricing.isHealthy ? "text-profit" : "text-loss"}`}>
              {t("product.marginPct", { n: pricing.marginPercent.toFixed(1) })}
            </p>
            <p className="mt-1 text-xs text-muted">
              {pricing.isHealthy ? t("product.healthy") : t("product.unhealthy")}
            </p>
          </Link>
        );
      })}
